...
- `src/pages/mobile/NewTest.tsx` — 6-adım yeni test akışı (Operasyon → Numune → Açıklama → **Kit & Analiz birleşik** → Sonuç+Override → Tamamla). Kamerayı tek seferde aç → ZXing QR'ı eşleşir eşleşmez kit kimliği belirlenir, eşleşme tetikleyicisiyle iki useEffect zincirleme **otomatik** olarak `handleFotoCek` (panel paterni + kompozit görsel) ve `handleAnaliz`'ı çalıştırır, 1500-2400 ms içinde sonuç adımına geçer. Sonuç override'i ≥10 karakter gerekçe ister, kayıt sonrası `downloadTestRaporu` ile PDF indirilir.
...
- `src/lib/kitOverlayComposite.ts` — Kit fotoğrafı üzerine her panel için test penceresi çerçevesi + **gerçek kırmızı C/T çizgileri** çizen canvas kompozit. POZ/NEG yazılı etiketler YOK; çizgi varlığı/yokluğu görsel olarak yorumu belirler (drawTestLine + drawSideTag). Geçersiz kitlerde çerçeve amber, normalde cyan.
