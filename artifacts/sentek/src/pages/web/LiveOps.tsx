...
const LOKASYONLAR = [
  ...
  'Araç Arama Noktası', 'Antrepo Bölgesi', 'Havalimanı Kargo',
  'Mobil Saha Ekibi', 'Karayolu Kontrol Noktası', 'Posta / Kargo Merkezi',
];
...
const DEMO_OLAYLAR: Omit<AkisOlayi, 'id' | 'zaman'>[] = [
  ...
  { lokasyon: 'Laboratuvar', mesaj: 'SNT-LAB-2026-000002 numunesı analiz sürecine alındı', tip: 'lab' },
  { lokasyon: 'Mobil Saha Ekibi', mesaj: 'Numune sevk kaydı oluşturuldu — Liman\'a yolda', tip: 'sevk', operasyonNo: 'OPS-2026-0184' },
  { lokasyon: 'Stok Birimi', mesaj: 'STK-007 (AMP Kiti) kritik seviyede — 80 adet kaldı', tip: 'stok' },
  { lokasyon: 'Liman Kontrol Noktası', mesaj: 'Negatif test sonucu — konteyner yüzey sürüntüsü', tip: 'negatif', operasyonNo: 'OPS-2026-0185' },
  { lokasyon: 'Laboratuvar', mesaj: 'SNT-LAB-2026-000006 analiz raporu sisteme yüklendi', tip: 'lab' },
  ...
  { lokasyon: 'Karayolu Kontrol Noktası', mesaj: 'Düşük güven skoru analiz — manuel kontrol önerildi', tip: 'stok' },
  { lokasyon: 'Mobil Saha Ekibi', mesaj: 'Çevrimdışı modda 1 test kaydı oluşturuldu', tip: 'sistem' },
  { lokasyon: 'Laboratuvar', mesaj: 'SNT-LAB-2026-000004 teslim alındı — kabul edildi', tip: 'lab' },
];
...
function LokasyonKart({ lokasyon, testSayisi, pozitifSayisi }: { lokasyon: string; testSayisi: number; pozitifSayisi: number }) {
  const oran = testSayisi > 0 ? (pozitifSayisi / testSayisi) * 100 : 0;
  return (
    <div className="glass-card p-3">
      <div className="flex items-center gap-2 mb-2">
        <MapPin className="w-3 h-3 text-muted-foreground" />
        <p className="text-xs font-semibold text-foreground truncate">{lokasyon}</p>
      </div>
      <div className="flex items-center justify-between mb-1.5">
        <span className="text-xs text-muted-foreground">{testSayisi} test</span>
        <span className={`text-xs font-bold ${pozitifSayisi > 0 ? 'text-red-400' : 'text-muted-foreground'}`}>
          {pozitifSayisi} poz.
        </span>
      </div>
      <div className="w-full h-1.5 bg-secondary rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-all ${oran > 40 ? 'bg-red-500' : oran > 20 ? 'bg-amber-500' : 'bg-emerald-500'}`}
          style={{ width: `${Math.max(oran, oran > 0 ? 8 : 0)}%` }}
        />
      </div>
    </div>
  );
...
export default function LiveOps() {
  const { testKayitlari, labSevkler, stoklar, auditLogs } = useData();
  ...
  const getZaman = () => new Date().toISOString();

  // Mevcut verilerden ilk akış olaylarını oluştur
  useEffect(() => {
    const baslangic: AkisOlayi[] = [
      { id: 'init-1', zaman: new Date(Date.now() - 5000).toISOString(), lokasyon: 'Laboratuvar', mesaj: 'SNT-LAB-2026-000003 analiz sırasında — kimyasal test devam ediyor', tip: 'lab' },
      { id: 'init-2', zaman: new Date(Date.now() - 12000).toISOString(), lokasyon: 'Sınır Kapısı A', mesaj: 'Pozitif test kaydedildi — OPS-2026-0142', tip: 'pozitif', operasyonNo: 'OPS-2026-0142' },
      { id: 'init-3', zaman: new Date(Date.now() - 25000).toISOString(), lokasyon: 'Stok Birimi', mesaj: 'STK-002 kritik seviyede — 50 adet kaldı', tip: 'stok' },
      { id: 'init-4', zaman: new Date(Date.now() - 40000).toISOString(), lokasyon: 'Liman Kontrol Noktası', mesaj: 'LS-009 laboratuvara yolda — tahmini varış bekleniyor', tip: 'sevk' },
      { id: 'init-5', zaman: new Date(Date.now() - 60000).toISOString(), lokasyon: 'Merkez', mesaj: 'Sistem başlatıldı — Canlı operasyon izleme aktif', tip: 'sistem' },
    ];
    setAkis(baslangic);
  }, []);

  // Simüle edilmiş canlı akış
  useEffect(() => {
    if (!canli) return;
  ...
  const lokasyonStats = LOKASYONLAR.map(lok => ({
    lokasyon: lok,
    testSayisi: testKayitlari.filter(t => t.lokasyon === lok).length,
    pozitifSayisi: testKayitlari.filter(t => t.lokasyon === lok && t.testSonucu === 'Pozitif').length,
  })).filter(s => s.testSayisi > 0).sort((a, b) => b.testSayisi - a.testSayisi);
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-3">
          ...
          <div ref={akisRef} className="space-y-2 max-h-[520px] overflow-y-auto pr-1">
            <AnimatePresence mode="popLayout">
              {akis.map(olay => (
                <AkisKarti key={olay.id} olay={olay} />
              ))}
            </AnimatePresence>
            {akis.length === 0 && (
              <div className="glass-card p-8 text-center">
                <Radio className="w-8 h-8 text-muted-foreground/30 mx-auto mb-2" />
                <p className="text-sm text-muted-foreground">Akış başlatılmadı veya olay yok</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
              {sonPozitifler.map(kayit => (
                <div key={kayit.id} className="p-2.5 rounded-lg bg-red-500/5 border border-red-500/15">
                  ...
                  <p className="text-[10px] text-muted-foreground">{kayit.lokasyon}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          {kritikStok.length > 0 && (
            <div className="glass-card p-4">
              <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3 flex items-center gap-2">
                <Package className="w-3.5 h-3.5 text-amber-400" /> Kritik Stok
              </h3>
              <div className="space-y-2">
                {kritikStok.slice(0, 4).map(s => (
                  <div key={s.id} className="p-2.5 rounded-lg bg-amber-500/5 border border-amber-500/15">
                    <p className="text-xs font-semibold text-amber-400">{s.urunAdi}</p>
                    <p className="text-xs text-muted-foreground">
                      {s.durum === 'Tükendi' ? '⚠ Tükendi' : `${s.kalanAdedi} adet kaldı`}
                    </p>
                  </div>
                ))}
              </div>
          ...
          {/* Düşük Güvenli Analizler */}
          {dusukGuven.length > 0 && (
            <div className="glass-card p-4">
              <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3 flex items-center gap-2">
                <Shield className="w-3.5 h-3.5 text-violet-400" /> Düşük Güven Analizi
              </h3>
              <div className="space-y-2">
                {dusukGuven.slice(0, 3).map(t => (
                  <div key={t.id} className="p-2.5 rounded-lg bg-violet-500/5 border border-violet-500/15">
                    <p className="text-xs font-mono text-violet-400">{t.operasyonNo}</p>
                    <p className="text-xs text-muted-foreground">Güven: %{t.analizGuvenSkoru}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {sktYaklasan.length > 0 && (
            <div className="glass-card p-4">
              <h3 className="text-xs font-bold text-muted-foreground uppercase tracking-widest mb-3 flex items-center gap-2">
                <Clock className="w-3.5 h-3.5 text-orange-400" /> SKT Yaklaşıyor
              </h3>
              <div className="space-y-2">
                {sktYaklasan.slice(0, 3).map(s => {
                  const kalan = Math.ceil((new Date(s.skt).getTime() - Date.now()) / (24 * 60 * 60 * 1000));
                  return (
                    <div key={s.id} className="p-2.5 rounded-lg bg-orange-500/5 border border-orange-500/15">
                      <p className="text-xs font-semibold text-orange-400">{s.urunAdi}</p>
                      <p className="text-[10px] text-muted-foreground">{kalan} gün kaldı · {s.lotSeriNo}</p>
                    </div>
                  );
                })}
              </div>
      ...
      {/* Lokasyon Yoğunluk Haritası */}
      <div>
        <h2 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
          <MapPin className="w-4 h-4 text-primary" /> Lokasyon Bazlı Yoğunluk
        </h2>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2">
          {lokasyonStats.map(s => (
            <LokasyonKart key={s.lokasyon} {...s} />
          ))}
        </div>
      </div>
    </div>
